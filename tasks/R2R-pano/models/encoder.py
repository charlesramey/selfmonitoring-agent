...
class EncoderRNN(nn.Module):
    ...
    def __init__(self, opts, vocab_size, embedding_size, hidden_size, padding_idx,
        ...
        self.cell_type = opts.lang_embed
    ...
    def forward(self, inputs, lengths):
        ...
        if 'gru' in self.cell_type:
            if self.bidirectional:
                output_1, ht_1 = self.rnn(embeds, mask=embeds_mask)
                output_2, ht_2 = self.rnn(self.flip(embeds, 1), mask=self.flip(embeds_mask, 1))
                output = torch.cat((output_1, self.flip(output_2, 0)), 2)
                ht = torch.cat((ht_1, ht_2), 2)
            else:
                output, ht = self.rnn(embeds, mask=embeds_mask)
            return output.transpose(0,1), ht.squeeze(), embeds_mask
